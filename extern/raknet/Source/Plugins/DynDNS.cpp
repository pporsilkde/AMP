...
void DynDNS::Update()
{
    ...
    if (packet != nullptr)
    {
        if (connectPhase == CP_WAITING_FOR_DYNDNS_RESPONSE)
        {
            ...
            char *resultStr = strstr((char *) packet->data, "Connection: close");
        ...
        else
        {
            ...
            */
            ...
            char *resultStr = strstr((char *) packet->data, "Current IP Address: ");
            if (resultStr != nullptr)
            {
                resultStr += strlen("Current IP Address: ");
                SystemAddress myIp;
                myIp.FromString(resultStr);
