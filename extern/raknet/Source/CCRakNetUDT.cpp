...
#else
    if (!isContinuousSend && timeSinceLastTick > 100000)
...
uint64_t CCRakNetUDT::GetBytesPerSecondLimitByCongestionControl() const
{
    if (isInSlowStart)
        return 0;
#if CC_TIME_TYPE_BYTES == 4
...
BytesPerMicrosecond CCRakNetUDT::ReceiverCalculateDataArrivalRate(CCTimeType curTime) const
{
    ...
    */
    ...
    for (double arrival : packetArrivalHistory)
    {
        if (arrival >= oneEighthMedian && arrival < eightTimesMedian)
        {
...
BytesPerMicrosecond CCRakNetUDT::CalculateListMedianRecursive(const BytesPerMicrosecond inputList[CC_CRABNET_UDT_PACKET_HISTORY_LENGTH], int inputListLength, int lessThanSum, int greaterThanSum)
{
    ...
    for (int i = 1; i < inputListLength; i++)
    {
    ...
    else
    {
        ...
        return CalculateListMedianRecursive(lessThanMedian, lessThanMedianListLength,
                                            lessThanSum, greaterThanMedianListLength + greaterThanSum);
    }
}
...
// ----------------------------------------------------------------------------------------------------------------------------
bool CCRakNetUDT::OnGotPacket(DatagramSequenceNumberType datagramSequenceNumber, bool isContinuousSend, CCTimeType curTime,
...
// ----------------------------------------------------------------------------------------------------------------------------
void
CCRakNetUDT::OnSendAckGetBAndAS(CCTimeType curTime, bool *hasBAndAS, BytesPerMicrosecond *_B, BytesPerMicrosecond *_AS)
{
    if (curTime > lastTransmitOfBAndAS + SYN)
    {
        ...
        *hasBAndAS = *_AS != UNDEFINED_TRANSFER_RATE;
    }
    else
    {
        *hasBAndAS = false;
...
void CCRakNetUDT::UpdateWindowSizeAndAckOnAckPreSlowStart(double totalUserDataBytesAcked)
{
    ...
    double newCWND = totalUserDataBytesAcked / (double) MAXIMUM_MTU_INCLUDING_UDP_HEADER
...
{
    for (unsigned i = 0; i < CC_CRABNET_UDT_PACKET_HISTORY_LENGTH; i++)
    {
...
double CCRakNetUDT::GetRTT() const
{
    if (RTT == UNSET_TIME_US)
        return 0.0;
    return RTT;
