...
namespace MWMechanics
{
        ...
        const MWWorld::Ptr& actor)
    {
        ...
        EquipmentRequirementResult result;
        const char* scaleKey = nullptr;
        if (item.getTypeName() == typeid(ESM::Armor).name())
        {
            result = armorRequirement(item, actor);
            scaleKey = "armor requirement multiplier";
        }
        else if (item.getTypeName() == typeid(ESM::Weapon).name())
        {
            result = weaponRequirement(item, actor);
            scaleKey = "weapon requirement multiplier";
        }
        else
            return {};

        // ArenaMP exposes one predictable server-side level control for each
        // equipment family. It scales the final skill/attribute gate after the
        // automatic or legacy calculation, so admins can globally make weapons
        // or armor easier/harder without retuning every individual influence.
        // config.lua sends this category through ID_GAME_SETTINGS; local
        // settings.cfg values are overwritten on connection.
        if (result.mApplicable && scaleKey)
        {
            const float multiplier = std::clamp(getFloat(scaleKey, 1.f), 0.f, 5.f);
            result.mSkillRequired = quantizeRequirement(result.mSkillRequired * multiplier);
            result.mAttributeRequired = quantizeRequirement(result.mAttributeRequired * multiplier);
            fillActorValues(result, actor);
        }
        return result;
    }
