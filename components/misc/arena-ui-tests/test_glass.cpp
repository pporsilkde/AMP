...
int main(int argc, char** argv)
{
    ...
    if (updater.contentsMargins().top() != 52) return 2;
    updater.present();
    ...
    for (int i = 0; i < names.size(); ++i)
    {
        ...
        (new QListWidgetItem(icon, labels[i], tabs))->setSizeHint(QSize(162, 68));
    }
    ...
    if (central->geometry().top() < 52) return 9;
    ...
    wizard.show();
    app.processEvents();
    if (page->mapTo(&wizard, QPoint()).y() < 52) return 10;
