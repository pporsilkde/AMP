#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ArenaMP X053 static harness (supersedes X052).
...
for key in ('emoji font', 'menu font', 'chat font resource'):
    check(re.search(r'^%s\s*=' % re.escape(key), settings, re.M) is not None,
          'settings-default.cfg has no [Chat] "%s" key, '
          'Settings::Manager::getString would throw at startup' % key)
    check('Settings::Manager::getString("%s", "Chat")' % key in cpp,
          'GUIChat never reads the "%s" setting' % key)

# --- 9. the generated colour atlas is present and self-consistent ---
ATLAS_XML = 'files/mygui/ArenaMPChatColor.xml'
ATLAS_PNG = 'files/mygui/ArenaMPChatColor.png'
check(os.path.exists(ATLAS_PNG), 'the colour emoji atlas texture is missing')
if os.path.exists(ATLAS_XML):
    atlas = read(ATLAS_XML)
    xml.dom.minidom.parseString(atlas)
    check('ResourceManualFont' in atlas,
          'the atlas must be a ResourceManualFont; MyGUI cannot colour a TrueType font')
    for tag in ('cursor', 'selected', 'selected_back', 'substitute'):
        check('index="%s"' % tag in atlas,
              'the atlas has no "%s" control glyph, MyGUI needs it' % tag)
    codes = set(int(m) for m in re.findall(r'index="(\d+)"', atlas))
    for cp in (0x41, 0x410, 0x1F602):
        check(cp in codes,
              'the atlas is missing U+%04X; a chat font without it would lose '
              'ordinary text, since MyGUI has no font fallback' % cp)
    palette_block = re.search(r'const EmojiSlot sEmojiPalette\[\] = \{(.*?)\};', cpp, re.S)
    if palette_block:
        for esc in re.findall(r'"((?:\\x[0-9A-Fa-f]{2})+)"', palette_block.group(1)):
            raw = bytes(int(b, 16) for b in re.findall(r'\\x([0-9A-Fa-f]{2})', esc))
            cp = ord(raw.decode('utf-8'))
            check(cp in codes,
                  'palette code point U+%04X is not in the atlas, it would '
                  'render as a box' % cp)
    cmake = read('files/mygui/CMakeLists.txt')
    for f in ('ArenaMPChatColor.xml', 'ArenaMPChatColor.png'):
        check(f in cmake, '%s is not installed by files/mygui/CMakeLists.txt' % f)
check(os.path.exists('tools/arenamp_make_emoji_font.py'),
      'the atlas generator is missing')
...
if failures:
    print('X053 HARNESS FAILED (%d):' % len(failures))
    for item in failures:
        print('  - ' + item)
    sys.exit(1)

print('X053 harness passed: layout, localization, member order, protocol, settings, palettes, colour atlas.')
